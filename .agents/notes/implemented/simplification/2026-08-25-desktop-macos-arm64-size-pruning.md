...
The distributed ZIP passed strict code-sign verification, Stapler validation, Gatekeeper as a Notarized Developer ID app, first Runtime activation, Host startup, and an HTTP 200 Web response. A signed Beta 7 installation then discovered Beta 8 through the GitHub fallback, downloaded about 45.7 MB (31%) through the differential updater, installed it in place, activated the pruned Runtime, and returned HTTP 200. Unit coverage pins both the incompatible-prebuild removal and the rule that unrelated prebuild trees remain untouched.
